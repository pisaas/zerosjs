...
describe('authentication/jwt', () => {
  ...
  beforeEach(() => {
    ...
    return app.service('users').create({ name: 'David' }).then((res) => {
      user = res;
      return service.createAccessToken({}, { subject: `${user.id}` })
    }).then(res => {
      accessToken = res;
  ...
  it('getEntity', async () => {
    const [ strategy ] = app.service('authentication').getStrategies('jwt');
