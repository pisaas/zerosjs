...
describe('@zerosjs/authentication-client', () => {
  ...
  it('getFromLocation', async () => {
    const auth = app.authentication;
    let dummyLocation = { hash: 'access_token=testing' };
    ...
    dummyLocation = { search: 'access_token=testing' };
    token = await auth.getFromLocation(dummyLocation);

    assert.strictEqual(await auth.getFromLocation({}), null);
  ...
  it('authenticate, authentication hook, login event', () => {
    ...
    return promise.then(result => {
      ...
      return Promise.resolve(app.get('storage').getItem('zeros-jwt'));
    }).then(at => {
      assert.strictEqual(at, accessToken, 'Set accessToken in storage');
