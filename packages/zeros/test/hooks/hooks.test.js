...
describe('hooks basics', () => {
  ...
  it('works with services that return a promise (zeros-hooks#28)', () => {
    const app = zeros().use('/dummy', {
      get (id, params) {
        return Promise.resolve({ id, user: params.user });
      }
  ...
  describe('returns the hook object when passing true as last parameter', () => {
    ...
    it('on argument validation error (https://github.com/zerosjs/express/issues/19)', () => {
      const app = zeros().use('/dummy', {
        get (id) {
          return Promise.resolve({ id });
        }
    ...
    it('on error in error hook (https://github.com/zerosjs/express/issues/21)', () => {
      const app = zeros().use('/dummy', {
        get (id) {
  ...
  it('can register hooks on a custom method', () => {
    const app = zeros().use('/dummy', {
      ...
      other: zeros.activateHooks(['id', 'data', 'params'])(
        (id, data, params) => {
          return Promise.resolve([id, data, params]);
        }
      )
